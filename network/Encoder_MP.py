...
class Encoder_MP(nn.Module):
	...
	def __init__(self, H, W, message_length, blocks=4, channels=64):
		...
		# self.image_second_layer = MultiSpectralAttention(channels, H, W, reduction=16, freq_sel_method='lowf16')
		self.image_second_layer = MultiSpectralAttention(channels, H, W, reduction=16, freq_sel_method='midd16')
		# self.image_second_layer = MultiSpectralAttention(channels, H, W, reduction=16, freq_sel_method='high16')
		self.image_after_concat_layer = ConvBNRelu(2 * channels, channels)
	...
	def forward(self, image, message):
		...
		intermediate0 = self.image_second_layer(image_pre)
		intermediate1 = self.image_first_layer(image_pre)
		concat0 = torch.cat([intermediate0, intermediate1], dim=1)
		intermediate1 = self.image_after_concat_layer(concat0)
...
class Encoder_MP_Diffusion(nn.Module):
	...
	def forward(self, image, message):
		...
		intermediate0 = self.image_second_layer(image_pre)
		intermediate1 = self.image_first_layer(image_pre)
		concat0 = torch.cat([intermediate1, intermediate0], dim=1)
		intermediate1 = self.image_after_concat_layer(concat0)
