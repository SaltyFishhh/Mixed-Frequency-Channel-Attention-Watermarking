...
def save_images(saved_all, epoch, folder, resize_to=None):
	...
	diff_images_w_i = (watermarked_images - images + 1) / 2
	# diff_images = (noised_images - images + 1) / 2
	diff_images_n_w = (noised_images - watermarked_images + 1) / 2
	...
	# transform to rgb watermarked_images - images
	diff_images_linear_w_i = diff_images_w_i.clone()
	R = diff_images_linear_w_i[:, 0, :, :]
	G = diff_images_linear_w_i[:, 1, :, :]
	B = diff_images_linear_w_i[:, 2, :, :]
	diff_images_linear_w_i[:, 0, :, :] = 0.299 * R + 0.587 * G + 0.114 * B
	diff_images_linear_w_i[:, 1, :, :] = diff_images_linear_w_i[:, 0, :, :]
	diff_images_linear_w_i[:, 2, :, :] = diff_images_linear_w_i[:, 0, :, :]
	diff_images_linear_w_i = torch.abs(diff_images_linear_w_i * 2 - 1)

	# transform to rgb noised_image - watermarked_images
	diff_images_linear_n_w = diff_images_n_w.clone()
	R = diff_images_linear_n_w[:, 0, :, :]
	G = diff_images_linear_n_w[:, 1, :, :]
	B = diff_images_linear_n_w[:, 2, :, :]
	diff_images_linear_n_w[:, 0, :, :] = 0.299 * R + 0.587 * G + 0.114 * B
	diff_images_linear_n_w[:, 1, :, :] = diff_images_linear_n_w[:, 0, :, :]
	diff_images_linear_n_w[:, 2, :, :] = diff_images_linear_n_w[:, 0, :, :]
	diff_images_linear_n_w = torch.abs(diff_images_linear_n_w * 2 - 1)
	...
	# maximize diff in every image watermarked_images - images
	for id in range(diff_images_linear_w_i.shape[0]):
		diff_images_linear_w_i[id] = (diff_images_linear_w_i[id] - diff_images_linear_w_i[id].min()) / (
				diff_images_linear_w_i[id].max() - diff_images_linear_w_i[id].min())

	# maximize diff in every image noised_image - watermarked_images
	for id in range(diff_images_linear_n_w.shape[0]):
		diff_images_linear_n_w[id] = (diff_images_linear_n_w[id] - diff_images_linear_n_w[id].min()) / (
				diff_images_linear_n_w[id].max() - diff_images_linear_n_w[id].min())
	...
	stacked_images = torch.cat(
		[images.unsqueeze(0), watermarked_images.unsqueeze(0), noised_images.unsqueeze(0),
		 diff_images_linear_w_i.unsqueeze(0),
		 diff_images_linear_n_w.unsqueeze(0)], dim=0)
	...
	# encoded_filename = os.path.join(folder, 'epoch-{}-encoded.png'.format(epoch))
	# cover_images = images.mul(255).add_(0.5).clamp_(0, 255).to('cpu', torch.uint8).numpy()
	# saved_cover_image = Image.fromarray(np.array(cover_images, dtype=np.uint8)).convert("RGB")
	# saved_cover_image.save(encoded_filename)
